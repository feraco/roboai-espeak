...
if [[ ! -f "systemd_services/lex_agent_robust.service" ]]; then
    echo "❌ Error: systemd_services/lex_agent_robust.service not found. Run from roboai-espeak root directory."
    exit 1
fi
...
chmod +x scripts/lex_pre_start_checks_robust.sh
