...
class ASRProvider:
    ...
    def __init__(
        ...
        self.audio_stream: AudioInputStream = AudioInputStream(
            device_id=device_id,
            sample_rate=rate if rate else 16000,
            chunk_size=chunk if chunk else 1024
        )
        # Set the audio callback for websocket communication
        self.audio_stream.set_audio_callback(self.ws_client.send_message)
